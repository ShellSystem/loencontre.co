		...
		@for ($i = 1; $i < count($usuarios); $i++)
			<div class="panel panel-default">
				<div class="panel-heading">
					<p><strong>Correo electrónico: </strong>{{$usuarios[$i]->email}}</p>
				</div>
				<div class="panel-body">
					<p><strong>Username: </strong>{{$usuarios[$i]->name}}</p>
					<a class="enlace_modal btn" onclick="confirmar_eliminar_usuario({{$usuarios[$i]->id}})">Eliminar</a>
				</div>
				<div class="panel-footer">
					<p><strong>Nombre: </strong>{{$usuarios[$i]->user_name}}</p>
				</div>
			</div>
		@endfor
