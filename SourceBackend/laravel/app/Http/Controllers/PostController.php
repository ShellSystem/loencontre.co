...
class PostController extends Controller{
  ...
  public function addPost(Request $request){
    if($request->post){
      ...
      if (strcmp($line, "") !== 0){
        $response = $this->postValidation($line);
        if ($response[0] == 1) { # Paso la validacion          
          $post = new Post();
          $post->link = $response[1]['link'];
          $post->image = $response[1]['image'];
          ...
          if(!$this->repeated($post->link)){# No hay un post con el mismo link
            ...
            return json_encode(['error' => 'Ya existe un articulo con el mismo link']);  
          }
        }else{
      ...
      return json_encode(['error' => 'No se ha encontrado un parametro correcto']);
    }
  }
  ...
  public function postValidation($linea){
    ...
    $mensajes = [  
      ...
      'alpha_dash' => 'No se acepta el uso de caracteres especiales',
